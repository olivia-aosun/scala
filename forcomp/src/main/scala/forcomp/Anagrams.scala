...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(char => char).map(ltr => (ltr._1, ltr._2.size)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsHelper(occurrences: Occurrences): List[Occurrences] = occurrences match {
      case Nil => List(List())
      case x :: xs =>
        val subOfFirst = (for (i <- 1 to x._2) yield (x._1, i)).toList
        val possibleSubsAtCurLevel = for (a <- subOfFirst; b <- combinationsHelper(xs)) yield a :: b
        possibleSubsAtCurLevel ::: combinationsHelper(xs)
    }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(Nil)
      case _ => {
        val combs = combinations(occurrences).filter(x => dictionaryByOccurrences.contains(x))
        // list of all possible occurrence combinations that's valid in dictionary
        for {
          i <- combs
