...
object FunSets {
   ...
   */
    def filter(s: Set, p: Int => Boolean): Set = subset => contains(s, subset) && p(subset)
   ...
   */
  // mapping: there exists some element x in s only if there is an element y in the mapped set
  // such that f(y) = x
