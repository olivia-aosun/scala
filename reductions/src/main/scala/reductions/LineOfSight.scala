...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (index < input.length) {
      if (index == 0) output(index) = 0
        ...
        output(index) = max(input(index)/index, output(index - 1))
      }
      index += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while (index < until) {
      if (index == 0) maxAngle = 0
      else maxAngle = max(maxAngle, input(index) / index)
      index += 1
    }
    maxAngle
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    if (from == 0) {
      output(from) = 0
      downsweepSequential(input, output, 0, from + 1, until)
    } else {
      if (from < until) {
        val maxSoFar = max(startingAngle, input(from) / from)
        output(from) = maxSoFar
        downsweepSequential(input, output, maxSoFar, from + 1, until)
      }
    }
