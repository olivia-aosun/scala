...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, move) #:: tail => {
      val newlyExplored = explored + block
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(block, move), newlyExplored)
      newNeighbors ++ from(tail ++ newNeighbors, newlyExplored)
    }
  }
